...
const [query,setQuery] = useState("")
  ...
  const onSubmit = (data) =>{
    ...
    axios.post(`${process.env.REACT_APP_API_URL}/attendancemanage/attendance_active_hours`,Data,{
        headers:{
        ...
        <Table stickyHeader aria-label="sticky table" size="small">
          ...
          <TableBody>
            {!(Object.entries(rows).length === 0) ? (
              rows
              .filter((row)=>(category==="subcontractor")?row.subcontractor!==null:(category==="")?(true):(category!=="subcontractor" && category!=="")?(row.class===category):true)// filtering through different labour categories and sub contractor 
              .filter((obj) =>
